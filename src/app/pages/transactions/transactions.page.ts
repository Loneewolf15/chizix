...
getTransactions() {
  this.authService.getTransactions().subscribe((res: any) => {
    ...
    if (res.message === "Signature verification failed" && this.router.url !== '/auth-screen') {
      ...
      // Merge the four arrays into a single array
      const mergedArray = [...res.inapp, ...res.vtu, ...res.deposit, ...res.withdrawal];
      console.log(mergedArray);

      this.transactionx = mergedArray;
  ...
  filterTransactionx() {
    ...
    // Update receiver_name to be displayed for 'out' segment
      // this.transactionx.forEach(transaction => {
      //   transaction.receiver_name = transaction.sender_name === JSON.parse(localStorage.getItem('userData'))?.loginData.full_name
      //     ? transaction.receiver_name
      //     : transaction.receiver_name;
      // });
      
         // Swap sender_name and receiver_name for 'out' segment
    // this.transactionx.forEach(transaction => {
    //   const temp = transaction.sender_name;
    //   transaction.sender_name = transaction.receiver_name;
    //   transaction.receiver_name = temp;
    // });
