...
export class SendPage implements OnInit, OnDestroy {
  ...
  filterTag() {
    ...
    if (this.searchTerm.length >= 4) {
      this.filteredTags = this.tagName.filter((tag) =>
    ...
    this.authService.getTagnamex().subscribe(
      (response: any) => {
        if (
          ...
          const loggedInUserTag = localStorage.getItem("userData")
            ? JSON.parse(localStorage.getItem("userData")).loginData.user_tag
            : null;

          if (loggedInUserTag) {
            this.tagName = this.tagName.filter(
              (tag) => tag.user_tag !== loggedInUserTag
            );
          }
  ...
  setFocus() {
    ...
    const appColorMode = document.body.classList.contains("dark")
      ? "dark"
      : "light";
    ...
    parentElem.style.background =
      this.pin.length === 4 ? "var(--ion-color-base)" : "var(--ion-color-base)";
  }
  ...
  async presentSendAlert(
    ...
    switch (status) {
      ...
      case "failed":
        message =
          "Sorry!! <br> We could not complete your request at this time, Please enter an amount greater than ₦50.";
